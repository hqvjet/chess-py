...
pygame.init()
pygame.mixer.music.load('E:/Code/python_code/chess/PygameOnlineChess/Client/res/music/background.mp3')
...
def terminate():
    if player_key is not None:
        try:
            requests.get(SERVER_ADDRESS + 'close_session/{}'.format(player_key))
        except Exception:
            pass
    pygame.quit()
    sys.exit()
...
def load_image(name, colorkey=None):
    fullname = os.path.join('E:/Code/python_code/chess/PygameOnlineChess/Client/res/pictures/', name)
...
def board_init():
    global board
    board = Board(8, 8, COLOR, SERVER_ADDRESS)
...
def register(login, passwd):
    try:
        ok = requests.get(SERVER_ADDRESS +
...
def login_user(login, passwd):
    try:
        key = requests.get(SERVER_ADDRESS +
                           'login_user/{}\n{}'.format(login, passwd)).text
        if key == 'error':
...
def get_rating():
    rating = requests.get(SERVER_ADDRESS +
...
def find_opponent():
    ...
    try:
        board_is = requests.get(SERVER_ADDRESS +
                                'check_board_is/{}'.format(player_key)).text
        if board_is == 'OK':
            COLOR = int(requests.get(SERVER_ADDRESS +
                                     'get_color/{}'.format(player_key)).text)
            return True

        create = requests.get(SERVER_ADDRESS +
...
def check_result():
    result = requests.get(SERVER_ADDRESS +
